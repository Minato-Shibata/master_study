...
double g_m(double vmin,double v0,double ve, double vesc){
  ...
  vesc=vesc/v0;
  ve=ve/v0;
  double gauss=sqrt(3.1415926535897932);
  double result;
  if(vmin>=(ve+vesc)){
    result= 0.0;
  }else if(vmin<(vesc-ve)){
    result=(0.5*(-TMath::Erfc(vmin+ve)+TMath::Erfc(vmin-ve))-2.0*ve*exp(-sqrt(vesc))/gauss)/(ve*v0 *(TMath::Erf(vesc)-2/gauss*vesc*exp(-sqrt(vesc))));
  }else{
    result=(0.5*(-TMath::Erfc(vesc)+TMath::Erfc(vmin-ve))-(ve+vesc-vmin)*exp(-sqrt(vesc))/gauss)/(ve*v0 *(TMath::Erf(vesc)-2/gauss*vesc*exp(-sqrt(vesc))));
  }
...
double form_factor(double q,double A){
  ...
  temp=c*c+7.0/3.0*3.14*3.14*a*a-5.0*s*s;
  if(temp>=0.0){
    R=sqrt(temp);
  }else{
    R=0.0;
  ...
  return 3*(sin(q*R)-q*R*cos(q*R))/pow((q*R),3.0)*exp(-0.5*((q*s)*(q*s)));
...
void energy_speC1(){
  ...
  double r_O =4*M_D*M_O/(M_D+M_O)/(M_D+M_O);
  double c =299792.458;
  double E_0 =0.5*M_D*(v0/c)*(v0/c)*1e6;;
  double sigma_n=1.0e-5; //pb
  double sigma=sigma_n*(AO*mu_O/mun)*(AO*mu_O/mun);
  ...
  for(int i=1;i<=10000;i++){
    ...
    q=sqrt(2*M_O*Er);
    vmin=sqrt(Er/E_0/r_O)* v0;
    form =form_factor(q,AO);
    rate=R_0O/E_0/r_O*form*form*g_m(vmin/c,v0/c,ve_J/c,vesc/c)/2*v0/c*mass*16.0/150.0*sqrt(pi);
    //cout<<vmin<<endl;
  ...
  for(int i=1;i<=10000;i++){
    ...
    form =form_factor(q,AO);
    vmin=sqrt((Er/E_0/r_O))* v0;
    rate=R_0O/E_0/r_O*form*form*g_m(vmin/c,v0/c,ve_D/c,vesc/c)/2*v0/c*mass*16.0/150.0*sqrt(pi);
