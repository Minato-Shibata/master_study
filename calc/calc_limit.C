...
double g_m(double vmin,double v0,double ve, double vesc){
  ...
  vesc=vesc/v0;
  ve=ve/v0;
  double gauss=pow(3.1415926535897932,0.5);
  double result;
  if(vmin>=(ve+vesc)){
    result= 0.0;
...
void calc_limit(){
  ...
  double m = .350;  /////NaI mass 
  /////Visible energy = a+b*(bin)///
  double a=-21.0097; ///////CHANGE THIS
  double b=0.05913;  ////////CHANGE THIS
  ...
  for(int i=2;i<=32767;i++){
    ...
    //  if(er>0) riron->SetBinContent(i-1,spe(100.,er));
    
  }

  double bin_width=1.*b/q; /////Bin width;
  int comb =(int)(energy_width/bin_width);
  qene->Rebin(comb);
  ...
  for(i=0;i<point;i++){
	...
	if(max>buf3) max=buf3;
      }
      
    }
    sig[i]=1.0*max;
    if(sig[i]>1e-70) fprintf(fp,"%e %e\n",mass[i],sig[i]*1.0e-36);
  }
